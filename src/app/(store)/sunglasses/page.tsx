...
export const metadata: Metadata = {
  ...
  description: "Browse the complete SPECTRA luxury sunglasses collection. Handcrafted frames, polarized lenses.",
};
      ...
      <div className="relative border-b border-white/[0.08] bg-gradient-to-b from-[#141414] to-[#0a0a0a] py-10 sm:py-20">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
          ...
          <p className="text-neutral-400 text-[13px] sm:text-[14px] mt-3 max-w-md mx-auto">
            Engineered with aerospace-grade precision and high-contrast optical polarization.
          </p>
        </div>
      </div>
        ...
        <div className="hidden md:flex flex-col md:flex-row md:items-center justify-between gap-4 px-4 py-3 mb-8 rounded-sm">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-2 md:pb-0 scrollbar-none">
            {categories.map((c) => {
              ...
              const href = c === "all" ? "/sunglasses" : `/sunglasses?category=${c}`;
              return (
                <Link
                  key={c}
                  href={href}
          ...
          <div className="flex items-center gap-3 self-end md:self-auto">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] text-neutral-500 uppercase tracking-widest hidden sm:inline">Shape:</span>
              <div className="flex items-center gap-1 overflow-x-auto">
                {shapes.slice(0, 5).map((s) => {
                  const isActive = shapeFilter === s;
                  const href = s === "all"
                    ? (categoryFilter !== "all" ? `/sunglasses?category=${categoryFilter}` : "/sunglasses")
                    : `/sunglasses?shape=${s}${categoryFilter !== "all" ? `&category=${categoryFilter}` : ""}`;
                  return (
                    <Link
                      key={s}
                      href={href}
